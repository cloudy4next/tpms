...
<script>
  $(document).ready(function(){
    ...
    function fetch_all_contacts(){
      $.ajax({
          url:"{{ route('provider.fetch.all.contacts') }}",
          method:"POST",
    ...
    function fetch_all_patients(){
      $.ajax({
          url:"{{ route('provider.fetch.all.patients') }}",
          method:"POST",
          data:{'_token' : "{{csrf_token()}}"},
          dataType:"json",
          success:function(data){
    ...
    function existing_chats(){
      if(ajax_chat_check==true){
        $.ajax({
            url:"{{ route('provider.existing.chats') }}",
            method:"POST",
            data:{
              '_token' : "{{csrf_token()}}",
              'lc_time':lc_time
    ...
    function existing_msgs(){
      if(ajax_run_check==true){
        $.ajax({
            url:"{{ route('provider.existing.msgs') }}",
            method:"POST",
            data:{
              '_token' : "{{csrf_token()}}",
              "id": chat_id,
    ...
    function scroll_msgs(){
      ...
      $.ajax({
          url:"{{ route('provider.scroll.msgs') }}",
          method:"POST",
          data:{
            '_token' : "{{csrf_token()}}",
            "id": chat_id,
    ...
    function print_msgs(arr){
      ...
      $.each(arr, function(index, val) {
        ...
        if(val.attach!=null){
          url="{{route('provider.chat.download.file',':id')}}";
    ...
    function update_read_status(){
      $.ajax({
          url:"{{ route('provider.update.read.status') }}",
          method:"POST",
    ...
    function delete_chat(){
      ...
      // console.log(chat_id);
      $.ajax({
          url:"{{ route('provider.delete.chat') }}",
          method:"POST",
          data:{
            '_token' : "{{csrf_token()}}",
            "id": chat_id
    ...
    function delete_msg(id){
      if(ajax_delete_check==true){
        $.ajax({
          url:"{{route('provider.chat.delete.msg')}}",
          method:"POST",
          data:{
            "_token":"{{csrf_token()}}",
            "msg_id":id
    ...
    $(document).on('click','.side_link',function(){
      ...
      fl=$(this).attr("fl");
      let url = "{{ route('provider.chat.history', ':id') }}";
      if(chat_id==0){
    ...
    $('#attached_file').change(function(e){
      ...
      uploadAjax=$.ajax({
        ...
        url: "{{route('provider.chat.upload.file')}}",
        type: "POST",
        data: fd,
        processData: false,
        contentType: false,
