        ...
        <div class="iq-card-body">
            ...
            <div class="overflow-hidden mb-2">
                ...
                <div class="float-right">
                    <a href="{{ route('provider.client.authorization', $client_id->id) }}"
                       class="btn btn-sm btn-primary" title="Back To Authorization"><i
                            class="ri-arrow-left-circle-line"></i>Back</a>
                </div>
            </div>
            <form action="{{ route('provider.client.authorization.update') }}" method="post"
                ...
                <div class="row">
                        ...
                        <div class="ui-widget">
                            <select class="form-control form-control-sm" placeholder="Search" name="payor_id" required>
                                @foreach ($all_payors as $payor)
                                    ...
                                    ?>
                                    <option value="{{ $show_payor->id }}"
                                        {{ $edit_authorization->payor_id == $show_payor->id ? 'selected' : '' }}>
                                        {{ $show_payor->payor_name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-md-4 col-lg-3 mb-2">
                        ...
                        <div class="input-group">
                            ...
                            ?>
                            <input class="form-control form-control-sm reportrange" name="select_date"
                                   value="{{ $edit_authorization->selected_date }}" required>
                        </div>
                    </div>
                    <div class="col-md-4 col-lg-3 mb-2">
                        <label>Authorization Number <span class="text-danger">*</span></label>
                        <input type="text" class="form-control form-control-sm" name="authorization_number"
                               value="{{ $edit_authorization->authorization_number }}" required>
                    </div>
                    <div class="col-md-4 col-lg-3 mb-2">
                        <label>UCI / Insurance ID <span class="text-danger">*</span></label>
                        <input type="text" class="form-control form-control-sm" name="uci_id"
                    ...
                    <div class="col-md-4 col-lg-3 mb-2">
                        ...
                        @else
                            <input type="text" class="form-control form-control-sm" name="cms_four"
                                   value="{{ $edit_authorization->cms_four }}">
                        @endif
                    </div>
                    <div class="col-md-4 col-lg-3 mb-2">
                    ...
                    <div class="col-lg-2 align-self-start mt-2">
                        <button type="submit" class="btn btn-primary">Save Auth</button>
                ...
                <div class="table-responsive">
                    <table class="table-sm table table-bordered mt-2 mb-2 c_table">
                        ...
                        @foreach ($activities as $act)
                            ...
                            $scheduled_hrs_get = \App\Models\Appoinment::where('admin_id', Auth::user()->id)
                                ->where('client_id', $client_id->id)
                                ->where('authorization_activity_id', $act->id)
                                ->sum('time_duration');
                            $rendered_hrs_get = \App\Models\Appoinment::where('admin_id', Auth::user()->id)
                                ->where('client_id', $client_id->id)
                                ->where('authorization_activity_id', $act->id)
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($scheduled_hrs <= 1)
                                        @if ($scheduled_hrs > $act->hours_max_is_one)
                                            - {{ $scheduled_hrs }}
                                            @if ($act->hours_max_one == 1)
                                                Hr
                                            @elseif ($act->hours_max_one == 2)
                                    ...
                                    @else
                                        @if ($scheduled_hrs > $act->hours_max_is_one)
                                            {{ $scheduled_hrs }} @if ($act->hours_max_one == 1)
                                                Hrs
                                            @elseif ($act->hours_max_one == 2)
                                                Am
                                            @elseif ($act->hours_max_one == 3)
                                ...
                                <td>
                                    @if ($rendered_hrs <= 1)
                                        {{ $rendered_hrs }} @if ($act->hours_max_one == 1)
                                            Hr
                                        @elseif ($act->hours_max_one == 2)
                                            Am
                                        @elseif ($act->hours_max_one == 3)
                                ...
                                <td>
                                    @if ($rem_hours <= 1)
                                        {{ $rem_hours }} @if ($act->hours_max_one == 1)
                                            Hr
                                        @elseif ($act->hours_max_one == 2)
                                            Am
                                        @elseif ($act->hours_max_one == 3)
                                ...
                                <td>
                                    <a href="#editactivity{{ $act->id }}" data-toggle="modal"
                                       title="Edit"><i class="fa fa-pencil text-success mr-2 editActModal"
                                                       data-id="{{$act->id}}"
                                                    ...
                                                    <form
                                                        ...
                                                        <div class="row">
                                                            <div class="col-md-5 mb-2">
                                                                <label>Service</label>
                                                                <select
                                                                    class="form-control form-control-sm edit_activity_one"
                                                            ...
                                                            <div class="col-md-4 mb-2">
                                                                ...
                                                                <select
                                                                    class="form-control form-control-sm exit_activity_two"
                                                                    name="activity_two" required>
                                                                    @foreach ($all_sub_acts as $sub_act)
                                                                        <option value="{{ $sub_act->sub_activity }}"
                                                                            {{ $act->activity_two == $sub_act->sub_activity ? 'selected' : '' }}>
                                                                            {{ $sub_act->sub_activity }}</option>
                                                                    @endforeach
                                                                </select>
                                                            ...
                                                            <div class="col-md-3 mb-2">
                                                                ...
                                                                <select
                                                                    ...
                                                                    @foreach ($cpt_cores as $cpt)
                                                                        <option value="{{ $cpt->cpt_id }}"
                                                                            {{ $act->cpt_code == $cpt->cpt_id ? 'selected' : '' }}>
                                                                            {{ $cpt->cpt_code }}</option>
                                                                    @endforeach
                                                                </select>
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td>
                                                                            <select
                                                                                ...
                                                                                name="hours_max_two">
                                                                                <option value="1"
                                                                                    {{ $act->hours_max_two == 1 ? 'selected' : '' }}>
                                                                                    Hours
                                                                                </option>
                                                                                <option value="3"
                                                                                    {{ $act->hours_max_two == 2 ? 'selected' : '' }}>
                                                                                    Unit
                                                                                </option>
                                                                            </select>
                                                                        </td>
                                                                        ...
                                                                        <td>
                                                                            <select
                                                                                ...
                                                                                <option value="Day"
                                                                                    {{ $act->hours_max_per_two == 'Day' ? 'selected' : '' }}>
                                                                                    Day
                                                                                </option>
                                                                                <option value="Week"
                                                                                    {{ $act->hours_max_per_two == 'Week' ? 'selected' : '' }}>
                                                                                    Week
                                                                                </option>
                                                                                <option value="Month"
                                                                                    {{ $act->hours_max_per_two == 'Month' ? 'selected' : '' }}>
                                                                                    Month
                                                                                </option>
                                                                                <option value="Total Auth"
                                                                                    {{ $act->hours_max_per_two == 'Total Auth' ? 'selected' : '' }}>
                                                                                    Total Auth
                                                                                </option>
                                                                            </select>
                                                                        </td>
    ...
    <div class="modal fade" id="addactivity" data-backdrop="static">
        <div class="modal-dialog  modal-dialog-scrollable modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('provider.client.authorization.ativity.save') }}" method="post"
                          autocomplete="off">
                        @csrf
                        <div class="row">
                            <div class="col-md-5 mb-2">
...
@endsection
@include('provider.client.include.authEdit')
