...
<div class="treatment-plan">
    <div class="content">
        ...
        <form action="{{route('provider.sn.eleven.form.submit')}}" method="post" id="cn_eleven_from">
            @csrf
            ...
            <div class="top-part" style="margin-top:10px;">
                ...
                <div class="row1 flex-div mb_30">
                    <div class="client-name"><label>Therapist:</label> <input type="text"
                                                                              placeholder="Enter Your Name..."
                                                                              name="terp"
                                                                              value="{{$form_eleven->terp}}"></div>
                    <div class="date"><label>Start Time:</label> <input type="time" name="sttitme" class="sttitme" value="{{\Carbon\Carbon::parse($form_eleven->sttitme)->format('H:i:s')}}">
                    </div>
                    <div class="date"><label>End Time:</label> <input type="time" name="endtime" class="endtime" value="{{\Carbon\Carbon::parse($form_eleven->endtime)->format('H:i:s')}}"></div>
                </div>
                <div class="mb_30">
                    <label class="d-block mb-2">Notes</label>
                    <textarea class="form-control border" rows="3" placeholder="Enter Notes..."
                              name="notes">{!! $form_eleven->notes !!}</textarea>
                </div>
            </div>
                        ...
                        <td rowspan="3">
                            <div class="flex-div first"><span><label>Location:</label></span> <span>
											<textarea class="form-control"
                                                      ...
                                                      name="location">{!! $form_eleven->location !!}</textarea></span>
                            </div>
                        </td>
                        <td><label>Date:</label><input type="date" name="lodate" value="{{$form_eleven->lodate}}"></td>
                    </tr>
                    <tr>
                        <td><label>Start Time:</label> <input type="time" name="losttime" class="losttime" value="{{\Carbon\Carbon::parse($form_eleven->losttime)->format('H:i:s')}}"></td>
                    </tr>
                    <tr>
                        <td><label>End Time:</label> <input type="time" name="loendtime" class="loendtime" value="{{\Carbon\Carbon::parse($form_eleven->loendtime)->format('H:i:s')}}"></td>
                    </tr>
                    <tr>
                        <td>
                            <div class="flex-div">
										<span><label>Location of
											...
											<textarea class="form-control"
                                                      ...
                                                      name="los">{!! $form_eleven->los !!}</textarea></span>
                            </div>
                        </td>
                        <td>
                            ...
                            <div class="form-check-inline">
                                <label class="form-check-label">
                                    <input type="radio" class="form-check-input" name="spyn"
                                           value="1" {{$form_eleven->spyn == 1 ? 'checked' : ''}}>Yes
                                </label>
                            </div>
                            <div class="form-check-inline">
                                <label class="form-check-label">
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="flex-div"><span><label>Strategies used during session:
                                                                            ...
                                                                            name="suds">{!! $form_eleven->suds !!}</textarea></span>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <div class="flex-div"><span><label>Targets worked on during session:
                                                                            ...
                                                                            placeholder="Enter here..."
                                                                            name="twods">{!! $form_eleven->twods !!}</textarea></span>
                            </div>
                        </td>
                        <td>
                            <div class="flex-div"><span><label>What
												...
												observed?
                                                                            ...
                                                                            placeholder="Enter here..."
                                                                            name="wnmbwo">{!! $form_eleven->wnmbwo !!}</textarea></span>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <div class="flex-div"><span><label>If "Other",
												...
												behaviors:
                                                                            ...
                                                                            placeholder="Enter here..."
                                                                            name="iopdb">{!! $form_eleven->iopdb !!}</textarea></span>
                            </div>
                        </td>
                        <td>
                            <div class="flex-div"><span><label>Notes:
                                                                            ...
                                                                            placeholder="Enter here..."
                                                                            name="note2">{!! $form_eleven->note2 !!}</textarea></span>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2">
                            <div class="flex-div"><span><label>Location
												Address:
                                                                            ...
                                                                            placeholder="Enter here..."
                                                                            name="ladd">{!! $form_eleven->ladd !!}</textarea></span>
                            </div>
                        </td>
                    </tr>
                    </tbody>
        ...
        <form class="pdf_form" action="{{ route('provider.print.form.11')}}" target="_blank" method="POST">
            @csrf
...
<script>
    $(document).ready(function () {
        ...
        $.ajax({
            type: "POST",
            url: "{{route('provider.sn.eleven.form.by.ajax')}}",
            data: {
                '_token': "{{csrf_token()}}",
                'sessionid': sessionid,
            },
        ...
        $(document).on('submit', '#cn_eleven_from', function (e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{route('provider.sn.eleven.form.submit')}}",
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
