<div class="col-md-4 pr-0">
    <h6 class="font-weight-bold">Address<i class="ri-question-line"
                                           title="Required for insurance billing—please use the client’s address they have on file with their insurance provider"></i>
        <span class="text-danger">*</span>
    </h6>
    <div class="row no-gutters">
        <div class="col-md-8 mb-2">
            <input type="text"
                   class="form-control form-control-sm street {{ $errors->has('client_street') ? ' is-invalid' : '' }}"
                   placeholder="Street" name="client_street" value="{{$client_id->client_street}}" required>
        ...
        <div class="col-md-4 pl-2 mb-2">
            <button type="button" class="btn btn-sm btn-primary " id="addAddressClient" title="Add"><i
                    class="ri-add-line"></i></button>
        </div>
        <div class="col-md-10 mb-2">
            <input type="text"
                   class="form-control form-control-sm city {{ $errors->has('client_city') ? ' is-invalid' : '' }}"
                   placeholder="City" name="client_city" value="{{$client_id->client_city}}" required>
        ...
        <div class="col-md-6 mb-2 pr-2">
            <div class="ui-widget">
                <select
                    class="form-control form-control-sm state {{ $errors->has('client_state') ? ' is-invalid' : '' }}"
                    placeholder="State" name="client_state" required>
                    <optgroup label="-US">
        ...
        <div class="col-md-4 mb-2">
            <input type="text"
                   class="form-control form-control-sm zip {{ $errors->has('client_zip') ? ' is-invalid' : '' }}"
                   placeholder="Zip" name="client_zip" value="{{$client_id->client_zip}}" required>
    ...
    @foreach($address as $addres)
        <div class="row no-gutters existsaddresssection">
            ...
            <div class="col-md-8 mb-2">
                <input type="text" class="form-control form-control-sm" placeholder="Street" name="street[]"
                       value="{{$addres->street}}" required>
                <input type="hidden" class="form-control form-control-sm" name="address_edit_id[]"
                       value="{{$addres->id}}">
            </div>
            <div class="col-md-4 pl-2 mb-2">
                <button class="btn btn-sm btn-danger deleteexistsaddress" data-id="{{$addres->id}}" type="button"
            ...
            <div class="col-md-4 mb-2">
                <input type="text" class="form-control form-control-sm" placeholder="Zip" name="zip[]"
                       value="{{$addres->zip}}" required>
            </div>
    ...
    <div class="row no-gutters">
        <div class="col-md-6 mb-2 pr-2">
            <h6 class="font-weight-bold">POS<i class="ri-question-line"
                                               title="The default office location where this client will typically be seen."></i><span
                    class="text-danger">*</span></h6>
            <select class="form-control form-control-sm" name="location" disabled>
                <option value="Main Office" {{$client_id->location == "Main Office" ? 'selected' : ''}}>Main Office
                </option>
                <option value="Telehealth" {{$client_id->location == "Telehealth" ? 'selected' : ''}}>Telehealth
                </option>
                <option value="Home" {{$client_id->location == "Home" ? 'selected' : ''}}>Home</option>
            </select>
        </div>
        <div class="col-md-4 mb-2">
            <h6 class="font-weight-bold">Region<i class="ri-question-line"
                                                  title="The default office location where this client will typically be seen."></i><span
                    class="text-danger">*</span></h6>
            <select class="form-control form-control-sm" name="zone" disabled>
                @foreach($box_32 as $zone)
                    <option
                        value="{{$zone->id}}" {{$client_id->zone == $zone->id ? 'selected' : ''}}>{{$zone->zone_name}}</option>
                @endforeach
        ...
        <div class="col-md-6 mb-3">
            ...
            <select class="form-control form-control-sm" name="default_tz" required>
                <option value="0"
                        disabled {{$client_id->timezone==null?'selected':''}}>Set a
                    default timezone
                </option>
                @foreach($tz_list as $tz)
                    <option value="{{$tz->name}}" {{$client_id->timezone==$tz->name?'selected':''}}>{{'('.$tz->diff_from_gtm.') '.$tz->name}}</option>
                @endforeach
            </select>
        </div>
    </div>
