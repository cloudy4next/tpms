...
@section('client')
    <div class="iq-card">
        ...
        <div class="iq-card-body">
            <h5 class="mb-2">
                <a href="{{ route('client.myauthorization') }}" title="Back To Client"><i
                        class="ri-arrow-left-circle-line text-primary"></i> </a>
                <a href="{{ route('client.myauthorization') }}"
                ...
                <div class="table-responsive">
                    <table class="table-sm table table-bordered mt-2 mb-2 c_table">
                        ...
                        @foreach ($activities as $act)
                            ...
                            <?php
                            $scheduled_hrs_get = \App\Models\Appoinment::where('admin_id', Auth::user()->admin_id)
                                ->where('client_id', $client_id->id)
                                ->where('authorization_activity_id', $act->id)
                                ->sum('time_duration');
                            $rendered_hrs_get = \App\Models\Appoinment::where('admin_id', Auth::user()->admin_id)
                                ->where('client_id', $client_id->id)
                                ->where('authorization_activity_id', $act->id)
                                                        ...
                                                        <div class="row">
                                                            ...
                                                            <div class="col-md-4 mb-2">
                                                                ...
                                                                <select
                                                                    ...
                                                                    $trett_types = \App\Models\Treatment_facility::where('admin_id', Auth::user()->admin_id)->where('id', $t_type->treatment_type_id)->get();
                                                                    $array = [];
                                                                    foreach ($trett_types as $types) {
                                                                        array_push($array, $types->id);
                                                                    }

                                                                    $sub_acts = \App\Models\all_sub_activity::whereIn('facility_treatment_id', $array)->where('admin_id', Auth::user()->admin_id)->get();

                                                                    ?>

                                                                    @foreach ($sub_acts as $sub_act)
                                                            ...
                                                            <div class="col-md-3 mb-2">
                                                                ...
                                                                <?php
                                                                $t_type = \App\Models\Client_authorization::where('id', $act->authorization_id)->first();
                                                                $trett_types = \App\Models\Treatment_facility::where('admin_id', Auth::user()->admin_id)->where('treatment_name', $t_type->treatment_type)->get();
                                                                $array = [];
                                                                foreach ($trett_types as $types) {
                                                                    array_push($array, $types->id);
                                                                }
                                                                ...
                                                                $cpt_codes = \App\Models\setting_cpt_code::whereIn('facility_treatment_id', $array)->where('admin_id', Auth::user()->admin_id)->get();
...
@endsection
@include('client.info.include.editAuthJs')
