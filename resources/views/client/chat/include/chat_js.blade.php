...
<script>
    ...
    function fetch_all_contacts(){
      $.ajax({
          url:"{{ route('client.fetch.all.contacts') }}",
          method:"POST",
    ...
    function existing_chats(){
      if(ajax_chat_check==true){
        $.ajax({
            url:"{{ route('client.existing.chats') }}",
            method:"POST",
            data:{
              '_token' : "{{csrf_token()}}",
              'lc_time':lc_time
    ...
    function existing_msgs(){
      if(ajax_run_check==true){
        $.ajax({
            url:"{{ route('client.existing.msgs') }}",
            method:"POST",
            data:{
              '_token' : "{{csrf_token()}}",
              "id": chat_id,
    ...
    function print_msgs(arr){
      ...
      $.each(arr, function(index, val) {
        ...
        if(val.attach!=null){
          url="{{route('client.chat.download.file',':id')}}";
    ...
    function update_read_status(){
    	$.ajax({
          url:"{{ route('client.update.read.status') }}",
          method:"POST",
    ...
    function delete_msg(id){
      if(ajax_delete_check==true){
        $.ajax({
          url:"{{route('client.chat.delete.msg')}}",
          method:"POST",
          data:{
            "_token":"{{csrf_token()}}",
            "msg_id":id
    ...
    $(document).on('click','.side_link',function(){
      ...
      let url = "{{ route('client.chat.history', ':id') }}";
    	fl=$(this).attr("fl");
      if(chat_id==0){
    ...
    $(document).on('submit','#upload_form',function(e){
      ...
      var formData = new FormData(this);
      if(formData.get('attached_file').name==''){
    ...
    $('#attached_file').change(function(e){
      ...
      uploadAjax=$.ajax({
        ...
        url: "{{route('client.chat.upload.file')}}",
        type: "POST",
        data: fd,
        processData: false,
        contentType: false,
