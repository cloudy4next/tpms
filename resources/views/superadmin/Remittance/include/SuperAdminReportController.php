...
class SuperAdminReportController extends Controller
{
    ...
    public function kpi_report_by_months(Request $request)
    {
        ...
        if (!$this->declare_PDF($d_from, $d_to, "month")) {
        ...
        $this->fpdf->SetWidths(array(23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 24));
        ...
        foreach ($date as $dat) {
            ...
            foreach ($lines as $line) {
                ...
                $t_ins_p = 0;
                if ($check_dep_data) {
                    $dep_payt_check = deposit::where('id', $check_dep_data->deopsit_id)->first();
                    if ($dep_payt_check->payor_type == 2) {
                        $total_ins_paid = deposit_apply::select('appointment_id', 'dos', 'amount', 'deopsit_id')
                            ->where('deopsit_id', $dep_payt_check->id)
                            ->where('appointment_id', $line->appointment_id)
                            ->where('dos', $line->schedule_date)
                            ->where('admin_id', $line->admin_id)
                            ->sum('payment');
                        $t_ins_p += $total_ins_paid;
                ...
                if ($check_dep_data) {
                    $dep_payt_check = deposit::where('id', $check_dep_data->deopsit_id)->first();
                    if ($dep_payt_check->payor_type == 1) {
                        $total_gran_paid = deposit_apply::select('appointment_id', 'dos', 'amount', 'deopsit_id')
                            ->where('deopsit_id', $dep_payt_check->id)
                            ->where('appointment_id', $line->appointment_id)
                            ->where('dos', $line->schedule_date)
                            ->where('admin_id', $line->admin_id)
                            ->sum('payment');
                        $t_g_p += $total_gran_paid;
                    } else {
                        $t_g_p += 0.00;
                    }
                } else {
                ...
                $final_ins_bal = $total_bal - $sum_pt_bal;
            ...
            $this->fpdf->Row(array($dat["y"], $dat["M"], $billed_amount, $adjustment, $allowed_amount, $gaurantor, $ins_paid, $total_paid, $patient_res, $ins_res, $balance, $percent), 0, $w);
        ...
        $this->fpdf->SetWidths(array(46, 23, 23, 23, 23, 23, 23, 23, 23, 23, 24));
        $this->fpdf->Row(array("Total", $t1, $t2, $t3, $t4, $t5, $t6, $t7, $t8, $t9, $tp), 4, $w);
        $path = public_path() . '/report/pdf/';
        $filename = "KPI_BY_MONTH_" . Auth::user()->id . '.pdf';
    ...
    public function kpi_report_by_patient(Request $request)
    {
        ...
        if (!$this->declare_PDF($d_from, $d_to, "patient")) {
            return back()->with('alert', "Please upload logo to proceed!");
            exit();
        }
        ...
        foreach ($p_ids as $id) {
            ...
            $this->fpdf->Row(array($p_name, $billed_amount, $adjustment, $allowed_amount, $gaurantor, $ins_paid, $total_paid, $patient_res, $ins_res, $balance, $percent), 5, $w);
        ...
        $path = public_path() . '/report/pdf/';
        $filename = "KPI_BY_PATIENT_" . Auth::user()->id . '.pdf';
    ...
    public function kpi_report_by_insurance(Request $request)
    {
        ...
        $i_ids = json_decode($request->insurance_id);
        ...
        if (!$this->declare_PDF($d_from, $d_to, "insurance")) {
            return back()->with('alert', "Please upload logo to proceed!");
            exit();
        }
        ...
        foreach ($i_ids as $id) {
            ...
            $this->fpdf->Row(array($i_name, $billed_amount, $adjustment, $allowed_amount, $gaurantor, $ins_paid, $total_paid, $patient_res, $ins_res, $balance, $percent), 5, $w);
        ...
        $path = public_path() . '/report/pdf/';
        $filename = "KPI_BY_INSURANCE_" . Auth::user()->id . '.pdf';
