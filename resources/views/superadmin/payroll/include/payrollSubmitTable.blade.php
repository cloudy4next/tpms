...
<table class="table table-sm table-bordered c_table" id="export_table">
    <thead>
    <tr>
        <th data-tableexport-display="none" class="checkbox1"><input type="checkbox" class="select_all_submit"></th>
    ...
    @foreach($query_exe as $submit_payroll)
        ...
        @if($bill_app)
            ...
            ?>
            <tr>
                <td data-tableexport-display="none"><input type="checkbox" class="select_in_submit" value="{{$submit_payroll->id}}"></td>
                <td>
                    @if($employee)
                        {{$employee->full_name}}
                    @endif
