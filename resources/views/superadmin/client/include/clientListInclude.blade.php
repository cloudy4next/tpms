    ...
    <thead>
        ...
        <tr class="bg-white">
            ...
            <th>
                ...
                <input class="form-control form-control-sm common_selector search_gender" type="search"
                    placeholder="Search Here">
            </th>
            <th>
                <input class="form-control form-control-sm common_selector search_sup" type="search"
                    placeholder="Search Here">
            </th>
            <th>
                <input class="form-control form-control-sm common_selector search_pos" type="search"
                    placeholder="Search Here">
            </th>
            <th>
                <input class="form-control form-control-sm common_selector search_insurance" type="search"
                    placeholder="Search Here">
            </th>
            <th></th>
            <th>
            ...
            <tr>
                <td class="text-left">
                    <a href="{{ route('superadmin.client.info', $client->id) }}"
                        class="mr-2">{{ $client->client_first_name }} {{ $client->client_middle }}
                        {{ $client->client_last_name }}</a>
                    {{-- <span --}}
                ...
                <td>
                    <?php
                    $client_auth = \App\Models\Client_authorization::where('admin_id', $admin_id)
                ...
                <td>
                    ...
                    @if ($client_autho)
                        <?php
                        if (Auth::user()->is_up_admin == 1) {
                            $show_payor = \App\Models\All_payor_detail::select('admin_id', 'payor_id', 'payor_name')
                                ->where('payor_id', $client_autho->payor_id)
                ...
                <td>
                    <?php
                        $primary_auth = \App\Models\Client_authorization::select('id')->where('client_id', $client->id)->where('is_primary',1)->first();
                        $current_date = \Carbon\Carbon::now()->format('Y-m-d');
                        $expired_auth = \App\Models\Client_authorization::select('id','end_date','is_valid')->where('client_id', $client->id)->where('end_date','<',$current_date)->where('is_valid',1)->first();

                        if(!$primary_auth){
                            $color = 'text-secondary';
                        }
                        else if($expired_auth){
                            $color = 'text-danger';
                        }
                        else{
                            $color = '';
                    ...
                    <a href="#myauth{{ $client->id }}" data-toggle="modal"><i class="fa fa-id-card-o {{$color}}" aria-hidden="true"></i></a>
                                    ...
                                    <table class="table table-sm c_table table-bordered">
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($all_authorizations as $authorization)
                                                ...
                                                ?>
                                                <tr>
                                                    <td>{{ $authorization->treatment_type }}
                                                        {{ \Carbon\Carbon::parse($authorization->start_date)->format('m/d/Y') }}
                                                        -{{ \Carbon\Carbon::parse($authorization->end_date)->format('m/d/Y') }}
                                                    </td>
                                                    <td>{{ \Carbon\Carbon::parse($authorization->start_date)->format('m/d/Y') }}
                                                    </td>
                                                    <td>{{ \Carbon\Carbon::parse($authorization->end_date)->format('m/d/Y') }}
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td><a
                                                            href="{{ route('superadmin.client.authorization', $client->id) }}">Go
                                                            To
                                                            Auth</a></td>
                                                </tr>
                                            @endforeach
                ...
                <td>
                    ...
                    <div class="custom-control custom-switch active-switch">
                        @if ($client->is_active_client == 1)
                            <input type="checkbox" class="custom-control-input clientactswt"
                                value="{{ $client->id }}" name="activeswitch" id="ac{{ $client->id }}" checked>
                            <label class="custom-control-label lblenameac" for="ac{{ $client->id }}">Active</label>
                        @else
                            <input type="checkbox" class="custom-control-input clientactswt"
                                value="{{ $client->id }}" name="activeswitch" id="ac{{ $client->id }}">
                            <label class="custom-control-label lblenameac"
                                for="ac{{ $client->id }}">In-Active</label>
                        @endif
