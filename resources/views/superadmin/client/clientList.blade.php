    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('change', '.clientactswt', function() {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('superadmin.clients.list.update.active') }}",
                    data: {
                        '_token': "{{ csrf_token() }}",
                        'client_id': client_id,
                        'is_active': is_active
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('superadmin.clients.list.get') }}",
                data: {
                    '_token': "{{ csrf_token() }}",
                },
                success: function(data) {
                    console.log(data);
                    if (data.is_success == 'done') {
                        $('.client_lists').empty();
            ...
            function filter_data() {
                ...
                var pos = get_filter('search_pos');
                var sup = get_filter('search_sup');
                var insurance = get_filter('search_insurance');
                var active_status = get_filter('active_status');
                $.ajax({
                    type: "POST",
                    url: "{{ route('superadmin.clients.list.get') }}",
                    data: {
                    ...
                    success: function(data) {
                        ...
                        if (data.is_success == 'done') {
                            ...
                            $('.search_pos').val(pos);
                            $('.search_sup').val(sup);
                            $('.search_insurance').val(insurance);
