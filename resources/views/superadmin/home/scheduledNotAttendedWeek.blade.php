...
@section('superadmin')
        ...
        <div class="iq-card-body">
            <div class="d-flex justify-content-between">
                <div class="align-self-center mb-2">
                    <h2 class="common-title m-0">Schedule Not Attended Last Week</h2>
                </div>
                <div class="align-self-center mb-2">
            ...
            <div class="table-responsive">
                <table class="table table-sm table-bordered c_table" id="export_table">
                    ...
                    <tbody>
                    @foreach($not_attended_last_week as $not_atten)
                        <?php
                        $client_name = \App\Models\Client::select('client_first_name', 'client_last_name')
                            ->where('id', $not_atten->client_id)
                            ->first();

                        $pro = \App\Models\Employee::select('id', 'admin_id', 'full_name')->where('id', $not_atten->provider_id)->first();
                        ?>
                        <tr>
                            <td>
                                @if ($client_name)
                            ...
                            <td>{{\Carbon\Carbon::parse($not_atten->schedule_date)->format('m/d/Y')}}</td>
                        </tr>
                    @endforeach
    ...
    <input type="hidden" value="NotAttended_" id="fileName">
