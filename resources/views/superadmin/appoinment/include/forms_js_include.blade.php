...
<script>
    ...
    $('.btnloading').hide();
    function submit_form(formData,formNum){

        var url=getUrl(formNum);
        if(url == "stop"){
            return false;
        }
        $('.btnloading').show();
        ...
        $.ajax({
            ...
            error: function (data) {
                toastr["error"]("Unknown error occrred!", 'ALERT!');
                $('#submit_btn').prop('disabled',false);
            }
        });
    }

    function getUrl(formNum){
        if(formNum == 1){
            return "stop";
        }
        else if(formNum == 2){
            return "{{route('superadmin.dsptn.two.form.submit')}}";
        }
        else{
            return "stop";
        }
