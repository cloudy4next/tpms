...
<body>
    <div class="treatment-plan">
        <div class="content">
                        ...
                        <tbody>
                            <tr>
                                <td>
                                    ...
                                    <textarea class="form-control" rows="3" placeholder="Enter here..." name="clinicstatus">{{ $data->clinicstatus }}</textarea>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <label class="text-primary">Who was present during the session</label>
                                    <textarea class="form-control" rows="3" placeholder="Enter here..." name="whopresent">{{ $data->whopresent }}</textarea>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <label class="text-primary">Behavior targeted for decrease</label>
                                    <textarea class="form-control" rows="3" placeholder="Enter here..." name="behatarget">{{ $data->behatarget }}</textarea>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <label class="text-primary">Techniques used during the session</label>
                                    <textarea class="form-control" rows="3" placeholder="Enter here..." name="techused">{{ $data->techused }}</textarea>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <label class="text-primary">Programs worked on</label>
                                    <textarea class="form-control" rows="3" placeholder="Enter here..." name="programwork">{{ $data->programwork }}</textarea>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <label class="text-primary">Reinforcers used</label>
                                    <textarea class="form-control" rows="3" placeholder="Enter here..." name="reinforce">{{ $data->reinforce }}</textarea>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <label class="text-primary">Client Progress</label>
                                    <textarea class="form-control" rows="3" placeholder="Enter here..." name="clientprogress">{{ $data->clientprogress }}</textarea>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <label class="text-primary">Plan for next session</label>
                                    <textarea class="form-control" rows="3" placeholder="Enter here..." name="plannext">{{ $data->plannext }}</textarea>
                                </td>
                            </tr>
                        </tbody>
                    </table>
            ...
            <form class="pdf_form" action="{{ route('superadmin.print.form.18') }}" target="_blank" method="POST">
                @csrf
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('submit', '#form_18', function(e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('superadmin.18.form.submit') }}",
                    data: formData,
                    cache: false,
                    contentType: false,
                    processData: false,
