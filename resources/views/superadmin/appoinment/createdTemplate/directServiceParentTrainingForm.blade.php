...
<body>
    <div class="parent-training">
        ...
        <div class="content">
            ...
            <form action="{{ route('superadmin.dsptn.two.form.submit') }}" method="post" id="dsptn_two_form">
                @csrf
                                ...
                                <td>
                                    <div class="flex-div"><span>
                                            ...
                                            <input type="text" id="child-name" name="childname"
                                                value="{{ $form_two->childname }}" class="childname">
                                            <input type="hidden" id="childname" name="sessionid" class="sessionid"
                                                value="{{ $session_id }}">
                                        </span></div>
                                </td>
                                <td colspan="2">
                                    <div class="flex-div"><span>
                                            ...
                                            <input type="text" id="attendens" name="attendens"
                                                value="{{ $form_two->childname }}" class="attendens">
                                        </span></div>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <div class="flex-div"><span>
                                            ...
                                            <input type="time" id="start-time" name="starttime" class="starttime"
                                                value="{{ \Carbon\Carbon::parse($form_two->starttime)->format('H:i:s') }}">
                                        </span></div>
                                </td>
                                <td>
                                    <div class="flex-div"><span>
                                            ...
                                            <input type="time" id="end-time" name="endtime" class="endtime"
                                                value="{{ \Carbon\Carbon::parse($form_two->endtime)->format('H:i:s') }}">
                                        </span></div>
                                </td>
                                <td>
                                    <div class="flex-div"><span>
                                            ...
                                            <input type="date" id="date" name="stdate"
                                                value="{{ $form_two->stdate }}" class="stdate">
                                        </span></div>
                                </td>
                            </tr>
                        </tbody>
                ...
                <section class="section_2 mb_30">
                    <div class="col-title mb_30">
                        ...
                        <span class="red">Check the box to the left of one or more goals that apply to this
                            session</span>
                    </div>
                    <table cellpadding="0" cellspacing="0">
                        <tr>
                            <td>
                                <input id="1st-row" type="checkbox" checked="checked" name="goals1"
                                    {{ $form_two->goals1 == 1 ? 'checked' : '' }} class="goals1" value="1">
                            </td>
                        ...
                        <tr>
                            <td>
                                <input id="2st-row" type="checkbox" name="goals2"
                                    {{ $form_two->goals2 == 1 ? 'checked' : '' }} class="goals2" value="1">
                            </td>
                            <td><label for="2st-row">Role played new procedure / technique</label></td>
                        </tr>
                        <tr>
                            <td>
                                <input id="3st-row" type="checkbox" name="goals3"
                                    {{ $form_two->goals3 == 1 ? 'checked' : '' }} class="goals3" value="1">
                            </td>
                            <td><label for="3st-row">Gave performance feedback to parent on implementation</label></td>
                        </tr>
                        <tr>
                            <td>
                                <input id="4st-row" type="checkbox" name="goals4"
                                    {{ $form_two->goals4 == 1 ? 'checked' : '' }} class="goals4" value="1">
                            </td>
                            <td><label for="4st-row">Modified / created new goal based on parent information</label>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <input id="5st-row" type="checkbox" name="goals5"
                                    {{ $form_two->goals5 == 1 ? 'checked' : '' }} class="goals5" value="1">
                            </td>
                            <td><label for="5st-row">Modeled protocol with child (if child present
                                    (0368T/0369T)</label>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <input id="6st-row" type="checkbox" name="goals6"
                                    {{ $form_two->goals6 == 1 ? 'checked' : '' }} class="goals6" value="1">
                            </td>
                ...
                <section class="section_2 mb_30">
                    <div class="box box1 mb_30">
                        <div class="col-title mb_15">
                            ...
                            <span class="red">What activities did you engage in to help the client move closer to
                                his/her goals
                                through
                                parent training? What materials did you use? A general summary will suffice.</span>
                        </div>
                        <div class="textarea">
                            <textarea id="activities" name="act" class="act" rows="5">{!! $form_two->act !!}</textarea>
                        </div>
                    </div>
                    <div class="box box2">
                        <div class="col-title mb_15">
                        ...
                        <div class="textarea">
                            <textarea id="needs" name="needs" class="needs" rows="5">{!! $form_two->needs !!}</textarea>
                        </div>
                    </div>
                </section>
            ...
            <form class="pdf_form" action="{{ route('superadmin.print.form.2') }}" target="_blank" method="POST">
                @csrf
    ...
    <script>
        $(document).ready(function() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('superadmin.dsptn.two.by.ajax') }}",
                data: {
                    '_token': "{{ csrf_token() }}",
                    'sessionid': sessionid,
                },
                success: function(data) {
                    ...
                    var st_time = moment(new Date(data.starttime)).format(
                        'HH:mm');
                    var end_time = moment(new Date(data.endtime)).format(
                        'HH:mm');


                    $('.starttime').val(st_time);
            ...
            $(document).on('submit', '#dsptn_two_form', function(e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('superadmin.dsptn.two.form.submit') }}",
                    data: formData,
                    cache: false,
                    contentType: false,
                    processData: false,
