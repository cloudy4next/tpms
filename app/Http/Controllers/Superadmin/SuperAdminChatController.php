...
class SuperAdminChatController extends Controller
{
    ...
    public function __construct()
    {
        $this->middleware(function ($request, $next) {
            ...
            $this->del_check=['both',Auth::user()->id.'-admin'];
            return $next($request);
        });
    }
            ...
            if($latest->id>$lm_id){
                ...
                foreach($msgs as $msg){
                    ...
                    if($msg->from_check==Auth::user()->id.'-'.$this->u_type){
                        $check="sent";
                        $admin_pro=admin_profile::where('admin_id',Auth::user()->id)->first();
                        if($admin_pro){
                            $profile=$admin_pro->profile_photo;
                            $profile_color=$admin_pro->profile_color;
                        }
    ...
    public function scroll_msgs(Request $request){
        ...
        })->orderBy('id','DESC')->limit(8)->get()->reverse();
    ...
    public function delete_chat(Request $request){
        ...
        if(strpos($data->is_del,'-')==false){
            $is_del=Auth::user()->id.'-'.$this->u_type;
            $data->is_del=$is_del;
        }
        else{
            $data->is_del="both";
    ...
    public function fetch_all_contacts(Request $request){
        ...
        $self=admin_profile::where('admin_id',Auth::user()->id)->first();
        if($self){
            ...
            $self=new admin_profile();
            $self->admin_id=Auth::user()->id;
            $self->profile_color=$this->colors[array_rand($this->colors)];
            $self->save();
        }
        ...
        if(Auth::user()->company_id!=null){
            $admins=Admin::where('id','!=',Auth::user()->id)->where('company_id',Auth::user()->company_id)->where('is_up_admin',1)->get();
        ...
        //SubAdmin
        if(Auth::user()->is_up_admin==1){
            $subadmins=Admin::where('up_admin_id',Auth::user()->id)->where('is_up_admin',2)->get();
        ...
        $employees=Employee::where('admin_id',$this->admin_id)->where('login_email','!=',NULL)->where('login_email','!=','')->get();
        ...
        //Clients
        $clients=Client::where('admin_id',$this->admin_id)->where('login_email','!=',NULL)->where('login_email','!=','')->get();
    ...
    public function fetch_all_patients(Request $request){
        ...
        //Clients
        $clients=Client::where('admin_id',$this->admin_id)->get();
        foreach($clients as $client){
            if($client->profile_color==null){
                $client->profile_color=$this->colors[array_rand($this->colors)];
                $client->save();
    ...
    public function generate_link(Request $request){
        ...
        $url=route('superadmin.client.info', $id);
        return response()->json([
            "status"=>"success",
            "url"=>$url
        ]);
    ...
    public function chat_history($id){
        ...
        return view('superadmin.chat.chatHistory',compact('data','user','fl','name'));
    }
