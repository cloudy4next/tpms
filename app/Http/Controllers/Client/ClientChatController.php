...
class ClientChatController extends Controller
{
    ...
    public function existing_chats(Request $request){
        ...
        if($lc_t>$lc_time){
            ...
            foreach($chats as $chat){
                ...
                $tz=Client::select('timezone')->where('id',Auth::user()->id)->first();
                if($tz){
                    $tz=$tz->timezone;
                    if($tz==null){
                        $tz='EST';
            ...
            if($latest->id>$lm_id){
                ...
                foreach($msgs as $msg){
                    ...
                    if($msg->from_check==Auth::user()->id.'-'.$this->u_type){
                        $check="sent";
                        $c_pro=Client::where('id',Auth::user()->id)->first();
                        if($c_pro){
                    ...
                    //     $hyperlink=route('client.client.info', $msg->hyperlink);
                    //     $client=Client::select('client_full_name','profile_photo')->where('id',$msg->hyperlink)->first();
    ...
    public function fetch_all_contacts(Request $request){
        ...
        $self=Client::where('id',Auth::user()->id)->first();
        ...
        //Admin
        $admins=Admin::where('id',$self->admin_id)->get();
        ...
        $pro_array=[];
        $providers=Appoinment::distinct()->select('provider_id')->where('client_id',Auth::user()->id)->get();
        foreach($providers as $provider){
            array_push($pro_array,$provider->id);
        }

        $employees=Employee::where('admin_id',$self->admin_id)->whereIn('id',$pro_array)->where('login_email','!=',NULL)->where('login_email','!=','')->get();
    ...
    public function chat_history($id){
        ...
        return view('client.chat.chatHistory',compact('data','user','fl','name'));
    }
